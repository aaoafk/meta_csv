...
module MetaCsv
  class Inferencer
      ...
      def infer_type_for_chunk chunk
        ...
        chunk.rows.headers.each do |header|
          ...
          chunk.rows[header].each do |v|
            ...
            when method(:matches_date_format?)
          ...
          values = values.map { |v| v.to_f / total }
    ...
    def infer_types_for_cells cells
      ...
      cells.each do |cell|
        ap cell.value
        inferred_types_for_headers[cell.header] = if /^[-+]?[0-9]*\.[0-9]+$/.match?(cell.value)
          Float
        elsif /^\d+$/.match?(cell.value)
          Integer
